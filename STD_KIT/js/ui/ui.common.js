...
/* Aside */
function asideInit(){
	$document.off('click.asideOpen').on('click.asideOpen', 'button.js_aside_opener', function(e){ asideOpen($(this).attr('aria-controls'), $(this)) });
	$document.off('click.asideClose').on('click.asideClose', 'button.js_aside_closer', function(e){ asideClose($(this).attr('aria-controls'), $(this)) });
}
...
function waveEffectInit(){
	...
	$document.off('mousedown.waveEffectEvent touchstart.waveEffectEvent').on('mousedown.waveEffectEvent touchstart.waveEffectEvent', ele, function(e) {
		...
		if (self.find(wave).length == 0){
			...
			self.find(wave).css({'top': y, 'left': x}).stop().animate({width: btnWidth * 3, height: btnWidth * 3 }, 400, function(){
				$(this).addClass('is_complete');
				if (events == 'mouseup'){
	...
	$document.off('mouseup.waveEffectEvent touchend.waveEffectEvent').on('mouseup.waveEffectEvent touchend.waveEffectEvent', ele, function(e) {
		...
		var self = $(this),
			wave = '.effect-wave';
		if (self.find(wave).hasClass('is_complete')){
			self.find(wave).stop().animate({opacity: '0'}, 200, function() {
				$(this).remove();
			})
		}
	});
	$document.off('click.waveEffectEvent focusin.waveEffectEvent').on('click.waveEffectEvent focusin.waveEffectEvent', function(e) {
		if ($('.effect-wave.is_complete').length){
			$('.effect-wave.is_complete').stop().animate({opacity: '0'}, 200, function() {
				$(this).remove();
			})
		}
	})
...
function dropOpen(id, callback){
	$('.drop-toggle.is-active').each(function(){ dropClose($(this).attr('aria-controls')) });
...
function tooltipInit(){
	$document.off('click.tooltipEvent').on('click.tooltipEvent', '.js-tooltip', function(e){
		tooltipOpen($(this).attr('aria-controls'));
	});
...
function tooltipOpen(id, callback){
	...
	$('button[aria-controls='+id+']').addClass('is-active').attr({'aria-expanded':'true'});
	setCallback(callback);
}
function tooltipClose(id, callback){
	$('#'+id).removeClass('is-active');
	$('button[aria-controls='+id+']').removeClass('is-active').attr({'aria-expanded':'false'});
	setCallback(callback);
}
...
function popoverInit(){
	$document.off('click.popoverEvent').on('click.popoverEvent', '.js-popover', function(e){
		popoverOpen($(this).attr('aria-controls'));
	});
...
function spyScrollInit(){
	$document.off('click.spyscrollEvent').on('click.spyscrollEvent', '.js-spyscroll', function(e){
		self.action($(this).attr('aria-controls'), $(this).data('scroll-target'), $(this).data('scroll-gap'));
	});
}
