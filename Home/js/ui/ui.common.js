...
var ui = {
	...
	gnb : {
		...
		$nav2_item: null,
		init: function(){
			this.$nav1_item = $('.gnb .nav1-item');
			this.$nav2_item = $('.gnb .nav2-item');
			if (this.$nav2_item.length) { this.event() }
		},
		set : function(n1, n2){
			var self = this;
			self.$nav1_item.eq(n1).addClass('is-current').siblings().removeClass('is-current');
			self.$nav1_item.eq(n1).find(self.$nav2_item).eq(n2).addClass('is-current').siblings().removeClass('is-current');
		},
		event: function(){
			var setTime = null;
			var self = this;
			self.$nav1_item.off('mouseenter focusin').on('mouseenter focusin', function(){
			...
			self.$nav1_item.off('mouseleave focusout').on('mouseleave focusout', function(){
				var $this = $(this);
	...
	aside: {
		...
		event: function(){
			var self = this;
			$document.off('click.asideOpen').on('click.asideOpen', '.header .btn-drawer', function(e){ self.open($(this)) });
			$document.off('click.asideClose').on('click.asideClose', '.aside .btn-close', function(e){ self.close() });
		},
		open: function($opener){
			var self = this;
			var $aside = $('.aside');
			$aside.data('opener', $opener);
			$opener.attr('aria-expanded', 'true');
			$aside.addClass('is-active').attr('aria-hidden', 'false');
			ui.dimmer.open($aside.find('.dimmer'));
		},
		close: function(){
			var self = this;
			var $aside = $('.aside');
			var $opener = $aside.data('opener');
			$opener.attr('aria-expanded', 'true');
			$aside.removeClass('is-active').attr('aria-hidden', 'true');
			ui.dimmer.close($aside.find('.dimmer'));
		},
	},
