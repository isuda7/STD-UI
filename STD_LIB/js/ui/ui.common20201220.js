...
var ui = {
	...
	setTabFocus: {
		enable: function($obj){
			var ele = '[tabindex=0], a, button, input, select';
			$document.off('focusin.setFocus').on('focusin.setFocus', function(e){
				if ($obj.has(e.target).length === 0){ $obj.find(ele).first().focus() }
			})
		},
		disable: function(){
			$document.off('focusin.setFocus');
		},
	},
	...
	tab: {
		...
		select: function(id, callback){
			...
			if ($eleTabPanel.length){ $eleTabPanel.addClass('is-active').attr('aria-hidden','false').siblings().removeClass('is-active').attr('aria-hidden','true') }
